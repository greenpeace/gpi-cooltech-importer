...
function upload_file() {
    ...
    $uploadedfile = $_FILES['file_excel'];
     ...
     */
    wp_die("non ha uploadato".$movefile['error'], $title, $args );
     //  echo $movefile['error'];
    }
...
<form enctype="multipart/form-data" method="post" action="options.php">
  ...
  ?>
	...
	<?php
	echo '<input type="file" name="file_excel" />';
  ...
  <?php $data=get_option('data');
	...
	?> <div> Last Update: <b> <?php echo date("d-m-Y", $data); ?></b></div>
</form>
<?php
 }
 ...
 function update_excel () {
    ...
    $objPHPExcel = $objReader->load($urlfinale);
    ...
    for ($row = 2; $row <= $highestRow; ++$row) {
          $post_type=addslashes($objWorksheet->getCellByColumnAndRow(10, $row)->getValue());
          ...
          foreach($subsectors as $su) {
            ...
            if($su=="Commercial-a") { $su="commercial-air-onditioning";}
          ...
          add_post_meta($id, "energy-efficency", $energy,true);
    }
